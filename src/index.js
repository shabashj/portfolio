...
$(document).ready(() => {
    ...
    const initText = [`Hi, I'm Evgeni and I specialize in Frontend Development since 2014.`,
        `Besides having good ping pong and code-debugging skills\n
I’m proficient in browser manipulation,\n
cross-browser compatibility and responsive design. \n
In my free time I play with new web frameworks and tools\n
and follow popular frontend trends.
...
In previous life: Developed for number of enterprises as SAP ABAP Developer.`
    ];
    ...
    function printWelcome() {
        let continueKey = isMobile() ? 'tap on screen' : 'press Enter';
        let welcomeText = `Welcome to my profile. \n To read more ${continueKey}.`;
        let charCounter = 0;
        let lines = welcomeText.split('\n');
        isPrinting = true;
        ...
        lines.forEach(function (line) {
            ...
            for (let i = 0; i < line.length; i++) {
                ...
                ((i) => delayedPrint = setTimeout(insertCharIntoLine.bind(this, lineElem, newDiv, line[i]),
                    charCounter++ * PRINT_DELAY))(i);
            }
        });
    }
    ...
    function addBlinker(newDiv) {
        let blinkElem = document.createElement('span');
    ...
    function skipAll() {
        ...
        let lastChar = $('.newLine .line').last();
        addBlinker(lastChar);
        removeListeners();
        scrollToBottom();
    }
    ...
    function countCharsInBlock(lines) {
        let lineCharsNum = 0;
        lines.forEach((line) => lineCharsNum = lineCharsNum + line.length);
        return lineCharsNum;
    }

    function clearEmptyLines(lines) {
        return lines.filter((val) => {
